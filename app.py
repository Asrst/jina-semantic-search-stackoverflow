...
def config():
    os.environ["JINA_DATA_FILE"] = os.environ.get(
        "JINA_DATA_FILE", "data/stackoverflow_sample.csv"
    )
    os.environ["JINA_WORKSPACE"] = os.environ.get("JINA_WORKSPACE", "workspace")

    os.environ["JINA_PORT"] = os.environ.get("JINA_PORT", str(45678))
    os.environ['JINA_PARALLEL'] = '2'
    os.environ['JINA_SHARDS'] = '1'
...
def print_topk(resp, sentence):
    for d in resp.search.docs:
        ...
        for idx, match in enumerate(d.matches):
            ...
            if score < 0.0:
                continue
            qid = match.meta_info.decode()
            dialog = match.text.strip()
            print(f'> {idx:>2d}({score:.2f}). {qid}-> "{dialog}"')
...
def query(top_k):
    ...
    with f:
        while True:
            text = input("please ask a question: ")
            if not text:
                break
